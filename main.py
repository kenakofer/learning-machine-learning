...
def new_layer_weights(layer_number):
    if layer_number < 0 or layer_number >= LAYER_COUNT:
        raise Exception(f"layer_number is {layer_number}. It must be between 0 and {LAYER_COUNT-1}")
    if layer_number == 0:
        return None
    return np.ones([NODES_PER_LAYER[layer_number], NODES_PER_LAYER[layer_number - 1]])
...
def feed_forward(inputs, weights, biases):
    ...
    if weights[0] != None:
        raise Exception("weights[0] should be None, since the input layer has no incoming weights.")
    ...
    # Set up the shape of the remaining layer activations based on shape of weights.
    # The correct activation values will feed forward
    activations.extend([ np.zeros(len(weights[l])) for l in range(1, layer_count) ])


    # For layer 1, all nodes j:
    for l in range(1, layer_count):
        z_values = np.array([
            sum(activations[l-1] * weights[l][j])
            for j in range(len(weights[l]))
        ])
        z_values += biases[l]
        activations[l] = sigmoid(z_values)
    return activations
...
# OLD SYSTEM
all_nodes = [[new_node(l) for _ in range(NODES_PER_LAYER[l])] for l in range(len(NODES_PER_LAYER))]

# NEW SYSTEM
all_weights = [new_layer_weights(l) for l in range(LAYER_COUNT)]
all_biases = [new_layer_biases(l) for l in range(LAYER_COUNT)]
